...
class GenericIngestor:
    ...
    def upsert(self, df, batch_id):
        ...
        df_cdc = (
            df_filtered.withColumn("row_number", row_number().over(windowSpec))
    ...
    def process_stream(self, df_stream):
        ...
        stream = (
            df_stream.writeStream.option(
                "checkpointLocation",
                f"/Volumes/raw/{self.schema}/cdc/postgres.public.{self.tablename}/{self.tablename}_checkpoint/",
            )
